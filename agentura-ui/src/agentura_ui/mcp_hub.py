...
class AgentConnection:
    ...
    base_url: str  # For file downloads
...
class MCPHub:
    ...
    async def connect_all(self) -> None:
        ...
        for agent in self._agents.values():
            logger.info("Connecting to %s at %s", agent.name, agent.url)
    ...
    async def discover(self) -> list[Tool]:
        ...
        Safe to call from asyncio.run() before Panel starts.
    ...
    async def call_tool(
        ...
        """Route a tool call to the correct agent. Reconnects if needed."""
