...
[![CI](https://github.com/semos-org/semos-agentura/actions/workflows/main.yml/badge.svg)](https://github.com/semos-org/semos-agentura/actions/workflows/main.yml)
...
| ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| **[semos-agentura-core](packages/semos-agentura-core/)**               | Shared framework: BaseAgentService, MCP+A2A transport, file middleware, LLMExecutor (universal agentic loop), reference client (AgenturaClient)                                                                   | Done    |
| **[Virtual Filesystem](packages/semos-agentura-files/src/semos/agentura/files/vfs.py)** | Unified API for local, WebDAV, SharePoint, and in-memory backends via fsspec. Single namespace for all file operations.                                                                                           | Done    |
...
| **[Orchestrator](docs/agent-architecture.md)**                         | Multi-session manager that can run independently in background. Operates on the virtual filesystem, maintains task lists and memory, spawns sub-agents. Email and UI are message paths into shared sessions.      | In work |

### User Interfaces

| Component                                    | Description                                                                                                                                                                                 | Status  |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| **[Agentura UI](packages/semos-agentura-ui/)**              | Multi-session, multi-model chat UI. Visualizes orchestrator state and is itself one message path into the orchestrator. Built on [panelini](https://github.com/opensemanticworld/panelini). | Partial |
| **[Email Agent](packages/semos-agentura-email/)** (as channel) | Second message path: incoming emails create/resume orchestrator sessions, replies go back via email. Uses `@mailgent` tagging.                                                              | Partial |

### Specialist Agents

| Agent                                     | Description                                                                                                                                                                                   | Tools | Status  |
| ----------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ------- |
| **[Filesystem Agent](packages/semos-agentura-files/)** | File operations on the Virtual Filesystem: read, write, edit, copy, move, search, glob, archive browsing across all backends.                                                                 | 19    | Done    |
| **[Document Agent](packages/semos-agentura-document/)**     | Non-plaintext document processing: OCR digest (PDF, images, Office), compose (Markdown to PDF/PPTX/DOCX/HTML), diagram generation (Mermaid, draw.io), form inspection and filling (PDF/DOCX). | 8     | Done    |
| **[Email Agent](packages/semos-agentura-email/)**           | Email and calendar operations: search, read, draft, reply, send. Calendar events and free slot computation. Backends: IMAP/SMTP, Outlook COM, MS Graph API.                                   | 10    | Done    |
...
```python
from semos.agentura.core import BaseAgentService, ToolDef, SkillDef, create_app
