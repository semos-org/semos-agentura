...
from semos.agentura.document.digestion._docx_digest import (
    _inline_images_as_base64,
    _normalize_pandoc_images,
    digest_docx_with_pandoc,
)
from semos.agentura.document.models import OutputMode
...
class TestDigestRoutingDocx:
    ...
    def test_auto_mode_uses_pandoc(self, sample_docx_rich: Path, tmp_path: Path):
        from semos.agentura.document.digestion.digest import digest
    ...
    def test_ocr_mode_skips_pandoc(self, sample_docx_rich: Path, tmp_path: Path):
        ...
        with patch("semos.agentura.document.digestion.digest.digest_docx_with_pandoc") as mock_pandoc:
            try:
                digest(
                    sample_docx_rich,
                    output_dir=tmp_path,
...
class TestDescribeImagesMock:
    ...
    def test_describe_images_calls_provider(self, tmp_path: Path):
        from semos.agentura.document.digestion._docx_digest import _annotate_images
        ...
        with patch(
            "semos.agentura.document.digestion._docx_digest.get_provider",
