...
import pytest
from semos.agentura.core.testing import mcp_client_for, parse_tool_result
from semos.agentura.document._utils import find_tool
from semos.agentura.document.models import DiagramResult
from semos.agentura.document.service import DocumentAgentService
...
async def test_generate_diagram_mocked(service):
    ...
    with patch("semos.agentura.document.tools.generate_diagram", new_callable=AsyncMock, return_value=mock_result):
        async with mcp_client_for(service) as client:
            result = await client.call_tool(
                "generate_diagram",
