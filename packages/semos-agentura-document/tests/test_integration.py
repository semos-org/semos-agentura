"""Tests requiring external tools (marp, pandoc, mmdc, libreoffice).

Each test has @needs_* markers for skipping when the tool is not installed.
These run in CI if the tool is available (pandoc is installed in CI).
"""
...
import pytest
from semos.agentura.document._utils import _project_root, find_tool
from semos.agentura.document.composition._slides import _find_browser
from semos.agentura.document.exceptions import ToolNotFoundError
from semos.agentura.document.models import OutputFormat
...
def _has_libre_office() -> bool:
    from semos.agentura.document.digestion._office import _find_libreoffice
...
def _drawio_can_render() -> bool:
    """drawio may be installed yet unable to render headless (e.g. a confined snap,
    or missing GL/display): it can exit 0 while producing no output. Probe an actual
    tiny export so such environments skip the rendering tests cleanly instead of
    failing on a non-functional binary. Runs only when drawio is present."""
    ...
    from semos.agentura.document.composition._drawio import render_drawio_to_png
...
class TestMermaidRendering:
    def test_render_to_png(self, tmp_dir):
        from semos.agentura.document.composition._mermaid import render_mermaid_to_png
    ...
    def test_render_to_base64(self, tmp_dir):
        from semos.agentura.document.composition._mermaid import render_mermaid_to_base64
    ...
    def test_replace_blocks(self, tmp_dir):
        from semos.agentura.document.composition._mermaid import replace_mermaid_blocks
...
class TestDrawioRendering:
    def test_render_to_png_with_embedding(self, tmp_dir):
        from semos.agentura.document.composition._drawio import (
...
class TestSlideGeneration:
    def test_html_output(self, tmp_dir):
        from semos.agentura.document.composition._slides import compose_slides
...
class TestDocumentGeneration:
    def test_docx_output(self, tmp_dir):
        from semos.agentura.document.composition._documents import compose_document
...
class TestOfficeConversion:
    def test_docx_to_pdf(self, sample_docx):
        from semos.agentura.document.digestion._office import convert_office_to_pdf
...
class TestCLI:
    def test_help(self):
        result = subprocess.run(
            ["uv", "run", "semos-agentura-document", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
    ...
    def test_digest_help(self):
        result = subprocess.run(
            ["uv", "run", "semos-agentura-document", "digest", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
            cwd=str(_project_root()),
    ...
    def test_inspect_docx(self, sample_docx):
        result = subprocess.run(
            ["uv", "run", "semos-agentura-document", "inspect", str(sample_docx)],
            capture_output=True,
            text=True,
            timeout=30,
            cwd=str(_project_root()),
